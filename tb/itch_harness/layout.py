...
The RTL exposes packed SystemVerilog structs as flat vectors under Verilator.
This file is the single Python-side definition of how to pack/unpack those
vectors.
...
def pack_data_t(event: dict[str, Any]) -> int:
    """Pack one events.jsonl record into the RTL data_t vector.

    This is used for order_book isolation tests where the golden normalised
    event is driven directly into the book RTL.
    """
    ...
    word = insert_field(
        word,
        DATA_FIELDS["updated_orn"],
        _none_to_zero(event.get("new_order_ref")),
    )
    word = insert_field(word, DATA_FIELDS["side"], _pack_side(event.get("side")))
    word = insert_field(word, DATA_FIELDS["shares"], _none_to_zero(event.get("shares")))
    word = insert_field(word, DATA_FIELDS["price"], _none_to_zero(event.get("price")))

    if word >= (1 << DATA_W):
        raise ValueError(f"packed data_t exceeds {DATA_W} bits: {word:#x}")

    return word


def unpack_data_t(word: int) -> dict[str, int]:
    """Unpack an RTL data_t vector into raw integer fields."""
