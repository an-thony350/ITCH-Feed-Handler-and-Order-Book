...
multi_pumped_bram #(.ADDRESS_W(BBO_W-6), .DATA_W(64)) bid_chunks (
    .bram_clk(bram_clk), .rst_n(rst_n),
    .rd_addr_a(bid_addr_a[BBO_W-1:6]), .rd_addr_b(bid_addr_b[BBO_W-1:6]),
    .rd_data_a(bid_chunk_a),           .rd_data_b(bid_chunk_b),
    .wr_we_a(cw_we_bid),  .wr_addr_a(cw_row_bid),  .wr_data_a(cw_dat_bid),
    .wr_we_b(cw2_we_bid), .wr_addr_b(cw2_row_bid), .wr_data_b(cw2_dat_bid)
);

multi_pumped_bram #(.ADDRESS_W(BBO_W-6), .DATA_W(64)) ask_chunks (
    .bram_clk(bram_clk), .rst_n(rst_n),
    .rd_addr_a(ask_addr_a[BBO_W-1:6]), .rd_addr_b(ask_addr_b[BBO_W-1:6]),
    .rd_data_a(ask_chunk_a),           .rd_data_b(ask_chunk_b),
    .wr_we_a(cw_we_ask),  .wr_addr_a(cw_row_ask),  .wr_data_a(cw_dat_ask),
    .wr_we_b(cw2_we_ask), .wr_addr_b(cw2_row_ask), .wr_data_b(cw2_dat_ask)
);

multi_pumped_bram #(.ADDRESS_W(BBO_W-6), .DATA_W(64)) shadow_bid_chunks (
    .bram_clk(bram_clk), .rst_n(rst_n),
    .rd_addr_a(next_target_chunk_idx), .rd_addr_b('0),
    .rd_data_a(current_bid_chunk),     .rd_data_b(),
    .wr_we_a(cw_we_bid),  .wr_addr_a(cw_row_bid),  .wr_data_a(cw_dat_bid),
    .wr_we_b(cw2_we_bid), .wr_addr_b(cw2_row_bid), .wr_data_b(cw2_dat_bid)
);

multi_pumped_bram #(.ADDRESS_W(BBO_W-6), .DATA_W(64)) shadow_ask_chunks (
    .bram_clk(bram_clk), .rst_n(rst_n),
    .rd_addr_a(next_target_chunk_idx), .rd_addr_b('0),
    .rd_data_a(current_ask_chunk),     .rd_data_b(),
    .wr_we_a(cw_we_ask),  .wr_addr_a(cw_row_ask),  .wr_data_a(cw_dat_ask),
    .wr_we_b(cw2_we_ask), .wr_addr_b(cw2_row_ask), .wr_data_b(cw2_dat_ask)
);
