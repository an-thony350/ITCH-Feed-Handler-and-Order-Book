"""Data contracts shared by the ITCH golden-model parser and order book."""
...
class NormalisedEvent:
    ...
    def __post_init__(self) -> None:
        ...
        if self.op == Op.ADD:
            assert self.side in (
                Side.BUY,
                Side.SELL,
            ), f"ADD at msg {self.msg_index} must have BUY or SELL side"
            ...
            assert (
                self.new_order_ref is None
            ), f"ADD at msg {self.msg_index} must not set new_order_ref"
            ...
            ), f"EXECUTE at msg {self.msg_index} must have UNKNOWN side"
            assert self.price is None, f"EXECUTE at msg {self.msg_index} must not set price"
            assert (
                self.shares is not None
            ), f"EXECUTE at msg {self.msg_index} missing shares"
            assert (
                self.new_order_ref is None
            ), f"EXECUTE at msg {self.msg_index} must not set new_order_ref"
            self._check_width("shares", self.shares, SHARES_BITS)

        elif self.op == Op.CANCEL:
            assert (
                self.side == Side.UNKNOWN
            ), f"CANCEL at msg {self.msg_index} must have UNKNOWN side"
            assert self.price is None, f"CANCEL at msg {self.msg_index} must not set price"
            assert (
                self.shares is not None
            ), f"CANCEL at msg {self.msg_index} missing shares"
            assert (
                self.new_order_ref is None
            ), f"CANCEL at msg {self.msg_index} must not set new_order_ref"
            self._check_width("shares", self.shares, SHARES_BITS)

        elif self.op == Op.DELETE:
            assert (
                self.side == Side.UNKNOWN
            ), f"DELETE at msg {self.msg_index} must have UNKNOWN side"
            assert self.price is None, f"DELETE at msg {self.msg_index} must not set price"
            assert (
                self.shares is None
            ), f"DELETE at msg {self.msg_index} must not set shares"
            assert (
                self.new_order_ref is None
            ), f"DELETE at msg {self.msg_index} must not set new_order_ref"

        elif self.op == Op.REPLACE:
            assert (
                self.side == Side.UNKNOWN
            ), f"REPLACE at msg {self.msg_index} must have UNKNOWN side"
            assert (
                self.price is not None
            ), f"REPLACE at msg {self.msg_index} missing new price"
            assert (
                self.shares is not None
            ), f"REPLACE at msg {self.msg_index} missing new shares"
            assert (
                self.new_order_ref is not None
            ), f"REPLACE at msg {self.msg_index} missing new_order_ref"
            self._check_width("price", self.price, PRICE_BITS)
            self._check_width("shares", self.shares, SHARES_BITS)
    ...
    def _check_width(self, field_name: str, value: int, width_bits: int) -> None:
        assert 0 <= value < (1 << width_bits), (
            f"{field_name} at msg {self.msg_index} must fit in {width_bits} bits, "
            f"got {value}"
        )
...
class Bbo:
    ...
    def __post_init__(self) -> None:
        assert (self.bid_price is None) == (
            self.bid_size is None
        ), "bid_price and bid_size must both be set or both be None"
        assert (self.ask_price is None) == (
            self.ask_size is None
        ), "ask_price and ask_size must both be set or both be None"

        if self.bid_price is not None and self.bid_size is not None:
            self._check_bbo_width("bid_price", self.bid_price, PRICE_BITS)
            self._check_bbo_width("bid_size", self.bid_size, SHARES_BITS)
        if self.ask_price is not None and self.ask_size is not None:
    ...
    def _check_bbo_width(self, field_name: str, value: int, width_bits: int) -> None:
        assert 0 <= value < (1 << width_bits), (
            f"{field_name} must fit in {width_bits} bits, got {value}"
        )
...
class Level:
    ...
    def __post_init__(self) -> None:
        ...
        assert (
            self.order_count >= 0
        ), f"Level order_count must be non-negative, got {self.order_count}"
        assert self.shares < (1 << SHARES_BITS), (
            f"Level shares must fit in {SHARES_BITS} bits, got {self.shares}"
        )
...
class BookState:
    ...
    def _check_levels(self, name: str, levels: dict[int, Level]) -> None:
        for price, level in levels.items():
            assert 0 <= price < (1 << PRICE_BITS), (
                f"{name} price must fit in {PRICE_BITS} bits, got {price}"
            )
            assert isinstance(level, Level), (
                f"{name}[{price}] must be a Level, got {type(level).__name__}"
            )
