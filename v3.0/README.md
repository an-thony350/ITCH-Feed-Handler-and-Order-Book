...
If you want to just run the processing system, the `v3release.bit` and `v3release.hwh` files are available for download and you can skip to the [Processing System Instructions](#processing-system-instructions)
...
- The board used for this design is **ZCU106**, or part **xczu7ev-ffvc1156-2-e**
...
```bash
git clone --no-checkout --single-branch --branch release-v3 https://github.com/an-thony350/ITCH-Feed-Handler-and-Order-Book
cd ITCH-Feed-Handler-and-Order-Book
git sparse-checkout init --cone
git sparse-checkout set v3.0
git checkout release-v3
```

2. Open Vivado 2023.2
3. In the Tcl Console, enter the following command: `cd <repository path>/v3.0`
...
The notebook used to run this can be found in [`v3.0/processing_system`](v3.0/processing_system). The following instructions should provide detail on how to use this system.
...
5. Using the notebook `v3_notebook.ipynb`, proceed to test the hardware and software designs, changing constants in the second cell where necessary
