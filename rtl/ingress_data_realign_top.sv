...
//
// Timing:
// - The Taxi-facing path remains 64-bit at 156.25 MHz.
// - No per-message padded AXI stream is recreated between MoldUDP64 parsing and
//   ITCH decoding.
// - The event output remains ready/valid so the eventual event CDC FIFO can
//   backpressure the decoder without losing a completed event.
...
module ingress_data_realign_top #(
  ...
  // AXIS Ethernet frame input from the MAC/testbench.
  ...
  // Normalised event output from the merged data_realign stage.
  output data_t m_event_data_o,
  output logic  m_event_valid_o,
  input  wire   m_event_ready_i,

  // MoldUDP64 sideband and sequence/gap status.
  ...
  data_realign u_data_realign (
    ...
    .ready_i              (m_event_ready_i),
    .rdata_o              (m_event_data_o),
    .valid_o              (m_event_valid_o),
