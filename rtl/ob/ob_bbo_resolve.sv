...
always_comb begin
    ...
    if(new_bbo_i) begin
        if(search_side_i) begin
            next_best_bid_o = bid_is_zero_i ? '0 : {target_chunk_idx_i, find_msb_bit(bid_active_chunks[target_chunk_idx_i])};
        end
        else begin
            next_best_ask_o = ask_is_zero_i ? BBO_W'(BBO_DEPTH-1) : {target_chunk_idx_i, find_lsb_bit(ask_active_chunks[target_chunk_idx_i])};
        end
    end
end
